...
def filter_doctors_logic(spec: str = None, max_f: int = None, min_exp: int = None, avail: bool = None): 
    ...
    if max_f is not None: 
        filtered = [d for d in filtered if d["fee"] <= max_f]
    if min_exp is not None: 
        filtered = [d for d in filtered if d["experience_years"] >= min_exp]
    if avail is not None: 
        filtered = [d for d in filtered if d["is_available"] == avail]
    return filtered